...
def check_cls_dataset(dataset: str, split=''):
    ...
    dataset = Path(dataset)#data的路径
    data_dir = (dataset if dataset.is_dir() else (DATASETS_DIR / dataset)).resolve()#dataset.is_dir()判断是不是一个目录
   #如果数据集存在，就读取，不存在创建，为数据集下载提供路径
    ...
    nc = len([x for x in (data_dir / 'train').glob('*') if x.is_dir()])  # 子目录数
