...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):  # detection layer
        ...
        self.reg_max = 16  # DFL channels (ch[0] // 16 to scale 4/8/12/16/20 for n/s/m/l/x)表示每个锚点输出的通道数；
        self.no = nc + self.reg_max * 4  # number of outputs per anchor
        self.stride = torch.zeros(self.nl)  # strides computed during build
        c2, c3 = max((16, ch[0] // 4, self.reg_max * 4)), max(ch[0], min(self.nc, 100))  # channels
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3), Conv(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch)#ch代表输出层的维度
        self.cv3 = nn.ModuleList(nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch)#15，18，21的维度
    ...
    def forward(self, x):
        ...
        if self.export and self.format in ('tflite', 'edgetpu'):
            ...
            img_h = shape[2] * self.stride[0]#H
            img_w = shape[3] * self.stride[0]#W
